...
script.js     dark mode, carousel, filters, modals, form
img/          all images, grouped by project
```

## How to add a project

1. Copy an existing `<article class="project-slide">` in `index.html`.
2. Set `data-modal` to a new id, for example `modal-newname`.
3. Set `data-categories` to one or more of: `product`  `ml`  `graphic`.
